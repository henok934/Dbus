...
from django.conf import settings  # Import settings for environment variables

# Twilio configuration
TWILIO_ACCOUNT_SID = settings.TWILIO_ACCOUNT_SID  # Use environment variable
TWILIO_AUTH_TOKEN = settings.TWILIO_AUTH_TOKEN      # Use environment variable
TWILIO_PHONE_NUMBER = settings.TWILIO_PHONE_NUMBER    # Use environment variable
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            message = client.messages.create(
                body=f'Ticket booked successfully for {firstname} {lastname} from {depcity} to {descity} on {date}.',
        ...
        return render(request, 'users/ticket.html', {
            'bus': bus,
            'des': des,
            'success': 'Ticket booked successfully! A confirmation SMS has been sent.'
        })
...
TWILIO_ACCOUNT_SID = 'ACa47d1b942e'
TWILIO_AUTH_TOKEN = 'de16fb97c5150b'
TWILIO_PHONE_NUMBER = '+251907252775'
def ticket(request):
    des = City.objects.all()
    bus = Bus.objects.all()
    if request.method == 'POST':
...
def send_sms(phone, message):
    # Hypothetical SMS API endpoint and parameters
    api_url = "https://api.your-sms-gateway.com/send"  # Replace with your SMS gateway URL
    payload = {
        'phone': phone,
        'message': message,
...
def ticket(request):
    # Fetch cities and buses for the dropdowns in the form
    des = City.objects.all()
    bus = Bus.objects.all()
    ...
    if request.method == 'POST':
        ...
        message = (
            ...
            f'Phone: {phone}\n'
            f'Departure City: {depcity}\n'
            f'Destination City: {descity}\n'
            f'Date: {date}\n'
            f'No. of Seats: {no_seat}\n'
            f'Price: {price}\n'
            f'Side No: {side_no}\n'
            f'Plate No: {plate_no}'
        )
        ...
        if sms_sent:
            ...
            success_message = 'Ticket booked successfully! However, SMS confirmation failed.'
...
def ticket(request):
    ...
    if request.method == 'POST':
        ...
        if sms_sent:
            success_message = 'Ticket booked successfully! A confirmation SMS has been sent.'
        else:
            success_message = 'Ticket booked successfully! However, SMS confirmation failed.'
